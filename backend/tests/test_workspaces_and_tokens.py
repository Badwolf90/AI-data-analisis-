...
async def test_workspace_team_invite_rbac_flow():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        login_res = await ac.post("/api/v1/auth/login", json={"email": "owner@acme.com", "password": "Password123!"})
        ...
        # Invite Member
        invite_res = await ac.post("/api/v1/workspaces/invites", json={"workspace_id": ws_id, "email": "member@acme.com", "role": "MEMBER"}, headers=headers_owner)
        ...
        login_member = await ac.post("/api/v1/auth/login", json={"email": "member@acme.com", "password": "Password123!"})
